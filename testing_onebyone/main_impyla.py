...
def main():
   ...
   time_s = time.time()
   for query in local_config_testing()['query_list']:
       with open(os.path.join(local_config_testing()['query_dir'],query),'r') as fp:
           conn = connect(host='172.168.0.24', port=21050,database = '%s'%(local_config_testing()['DATABASE_NAME']))
           cursor = conn.cursor()
           os.system("""
